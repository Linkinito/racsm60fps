...
int main(void) {
    ...
    /* Le dispatcher doit choisir un trampoline par callsite, traiter CUSTOM
       comme un repli TWO_PASS et détecter toute route connue mais incorrecte. */
    {
        const RcsmModuleProfile *reference = rcsm_find_module_profile(4u);
        RcsmModuleProfile runtime;
        RcsmCallsiteProfile relocated[RCSM_MAX_ACTIVE_CALLS];
        const uint32_t two_pass_hook = 0x08810000u;
        const uint32_t one_pass_hook = 0x08811000u;
        const uint32_t two_pass_word = rcsm_encode_jal(two_pass_hook);
        const uint32_t one_pass_word = rcsm_encode_jal(one_pass_hook);
        RcsmPatchResult result;
        RcsmRedirectState state;
        PatchMemory memory;
        size_t index;
        assert(reference != NULL);
        assert(rcsm_relocate_profile(reference, 0x8000, &runtime, relocated, RCSM_MAX_ACTIVE_CALLS));
        relocated[0].policy = RCSM_POLICY_ONE_PASS;
        relocated[1].policy = RCSM_POLICY_CUSTOM;
        relocated[2].policy = RCSM_POLICY_TWO_PASS;
        memset(&memory, 0, sizeof(memory));
        memory.count = runtime.callsite_count;
        for (index = 0u; index < memory.count; ++index) {
            memory.addresses[index] = runtime.callsites[index].ra - 8u;
            memory.words[index] = rcsm_encode_jal(runtime.two_pass_entry);
        }
        assert(rcsm_install_policy_redirects(
            &runtime, two_pass_hook, one_pass_hook,
            read_patch_word, write_patch_word, sync_patch_word, &memory, &result
        ));
        assert(memory.words[0] == one_pass_word);
        assert(memory.words[1] == two_pass_word);
        assert(memory.words[2] == two_pass_word);
        assert(rcsm_classify_policy_redirects(
            &runtime, two_pass_hook, one_pass_hook, read_patch_word, &memory, &state
        ));
        assert(state.redirected == runtime.callsite_count);
        assert(state.redirected_one_pass == 1u);
        assert(state.redirected_two_pass == runtime.callsite_count - 1u);
        assert(state.wrong_route == 0u);
        memory.words[0] = two_pass_word;
        assert(!rcsm_classify_policy_redirects(
            &runtime, two_pass_hook, one_pass_hook, read_patch_word, &memory, &state
        ));
        assert(state.wrong_route == 1u);
        memory.words[0] = one_pass_word;
        assert(rcsm_remove_policy_redirects(
            &runtime, two_pass_hook, one_pass_hook,
            read_patch_word, write_patch_word, sync_patch_word, &memory, &result
        ));
        assert(result.modified == runtime.callsite_count);
        for (index = 0u; index < memory.count; ++index) {
            assert(memory.words[index] == rcsm_encode_jal(runtime.two_pass_entry));
        }
    }

    printf("{\"status\":\"PASS\",\"modules\":%zu,\"callsites\":%zu,\"max_active\":%zu,\"anchors\":%zu,\"policy_redirects\":\"PASS\"}\n",
