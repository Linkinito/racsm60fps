#!/usr/bin/env python3
"""Test hôte du contrat trace v5, des politiques et du corpus."""
...
def main() -> None:
    ...
    entries = [
        {
            ...
            "configured_policy": (
                "one_pass" if row["mips_ra_runtime"].upper() == "0X092C142C"
                else "custom" if row["family_id"] == "WF-019"
                else "vanilla"
            ),
            "effective_policy": (
                "one_pass" if row["mips_ra_runtime"].upper() == "0X092C142C" else "two_pass"
            ),
            "custom_fallback": (
                row["family_id"] == "WF-019" and
                row["mips_ra_runtime"].upper() != "0X092C142C"
            ),
    ...
    trace = {
        ...
        "policy": {
            ...
            "two_pass_callsites": 1,
            "one_pass_callsites": 1,
            "custom_fallback_callsites": 1,
        },
        "capture": {
            "start_sample": 100,
            "end_sample": 200,
    ...
    with tempfile.TemporaryDirectory(prefix="rcsm-trace-test-") as directory:
        ...
        trace["entries"][active_index]["effective_policy"] = "two_pass"
        trace_path.write_text(json.dumps(trace), encoding="utf-8")
        ...
        assert "politique effective incohérente" in invalid_policy.stderr
        trace["entries"][active_index]["effective_policy"] = "one_pass"
    ...
    print(json.dumps({"status": "PASS", "trace_schema": 5, "entries": len(entries), "address_delta": address_delta}))
