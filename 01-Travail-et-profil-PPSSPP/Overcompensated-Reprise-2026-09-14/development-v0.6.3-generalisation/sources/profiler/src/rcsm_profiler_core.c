...
static int rcsm_policy_uses_one_pass(const RcsmCallsiteProfile *callsite) {
    return callsite != NULL && callsite->policy == RCSM_POLICY_ONE_PASS;
}

static int rcsm_policy_targets_valid(
    const RcsmModuleProfile *profile,
    uint32_t two_pass_hook_target,
    uint32_t one_pass_hook_target
) {
    return profile != NULL && two_pass_hook_target != 0u && one_pass_hook_target != 0u &&
           two_pass_hook_target != profile->two_pass_entry &&
           one_pass_hook_target != profile->one_pass_inner;
}

int rcsm_install_policy_redirects(
    const RcsmModuleProfile *profile,
    uint32_t two_pass_hook_target,
    uint32_t one_pass_hook_target,
    RcsmReadWord reader,
    ...
    if (!rcsm_validate_profile(profile) || reader == NULL || writer == NULL ||
        !rcsm_policy_targets_valid(profile, two_pass_hook_target, one_pass_hook_target)) {
        return 0;
    }
    ...
    for (index = 0; index < profile->callsite_count; ++index) {
        ...
        uint32_t target = rcsm_policy_uses_one_pass(&profile->callsites[index])
            ? one_pass_hook_target
            : two_pass_hook_target;
        ++local.examined;
        if (!rcsm_jal_reachable(site, target) || reader(site, &word, user) != 0 || word != original_word) {
            ++local.mismatched;
        }
    }
    ...
    for (index = 0; index < profile->callsite_count; ++index) {
        ...
        uint32_t target = rcsm_policy_uses_one_pass(&profile->callsites[index])
            ? one_pass_hook_target
            : two_pass_hook_target;
        uint32_t hook_word = rcsm_encode_jal(target);
...
int rcsm_remove_policy_redirects(
    const RcsmModuleProfile *profile,
    uint32_t two_pass_hook_target,
    uint32_t one_pass_hook_target,
    RcsmReadWord reader,
    ...
    RcsmPatchResult local = {0u, 0u, 0u, 0u};
    uint32_t original_word;
    uint32_t two_pass_hook_word;
    uint32_t one_pass_hook_word;
    size_t index;
    if (result != NULL) {
        *result = local;
    }
    ...
    original_word = rcsm_encode_jal(profile->two_pass_entry);
    two_pass_hook_word = rcsm_encode_jal(two_pass_hook_target);
    one_pass_hook_word = rcsm_encode_jal(one_pass_hook_target);
    for (index = 0; index < profile->callsite_count; ++index) {
        uint32_t word = 0u;
        uint32_t site = profile->callsites[index].ra - 8u;
        ++local.examined;
        ...
        if ((word != two_pass_hook_word && word != one_pass_hook_word) ||
            writer(site, original_word, user) != 0) {
            ++local.mismatched;
            continue;
        }
        ++local.modified;
...
int rcsm_classify_policy_redirects(
    const RcsmModuleProfile *profile,
    uint32_t two_pass_hook_target,
    uint32_t one_pass_hook_target,
    RcsmReadWord reader,
    ...
    RcsmRedirectState local = {0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u};
    uint32_t original_word;
    uint32_t two_pass_hook_word;
    uint32_t one_pass_hook_word;
    size_t index;
    ...
    for (index = 0; index < profile->callsite_count; ++index) {
        uint32_t word = 0u;
        uint32_t site = profile->callsites[index].ra - 8u;
        int expects_one_pass = rcsm_policy_uses_one_pass(&profile->callsites[index]);
        uint32_t expected_hook_word = expects_one_pass ? one_pass_hook_word : two_pass_hook_word;
        ++local.examined;
        if (reader(site, &word, user) != 0) {
            ++local.unreadable;
        } else if (word == original_word) {
            ++local.original;
        } else if (word == expected_hook_word) {
            ++local.redirected;
            if (expects_one_pass) {
                ++local.redirected_one_pass;
            } else {
                ++local.redirected_two_pass;
            }
        } else if (word == two_pass_hook_word || word == one_pass_hook_word) {
            ++local.wrong_route;
        } else {
            ++local.foreign;
        }
    ...
    return local.wrong_route == 0u && local.foreign == 0u && local.unreadable == 0u;
}
...
int rcsm_install_callsite_redirects(
    ...
    return rcsm_install_policy_redirects(
        profile, hook_target, hook_target, reader, writer, sync, user, result
    );
}
...
int rcsm_remove_callsite_redirects(
    ...
    return rcsm_remove_policy_redirects(
        profile, hook_target, hook_target, reader, writer, sync, user, result
    );
}
...
int rcsm_classify_callsite_redirects(
    ...
    return rcsm_classify_policy_redirects(
        profile, hook_target, hook_target, reader, user, state
    );
}
