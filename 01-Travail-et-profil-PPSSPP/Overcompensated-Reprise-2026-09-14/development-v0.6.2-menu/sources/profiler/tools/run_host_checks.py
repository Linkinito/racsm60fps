...
def main():
    ...
    reports=[]
    suites={
        'test_menu':['src/rcsm_menu.c'],
        'test_fps_choice':['src/rcsm_fps_choice.c'],
    ...
    with tempfile.TemporaryDirectory() as temp:
        for suite, sources in suites.items():
            exe=Path(temp)/(suite+('.exe' if os.name == 'nt' else ''))
            cmd=[args.cc,*args.cc_arg,'-std=c11','-Wall','-Wextra','-Werror','-Iinclude','-Igenerated']
