...
try{
 ...
 const start=0x09400000,end=0x0a000000;
 ...
 if(valid){const o=object-start;out.objects.push({name:item.name,descriptor,object,update:item.cb[2],pvars:data.readUInt32LE(o+0x58),props:data.readUInt32LE(o+0x54)});}}
 fs.writeFileSync(path.join(root,'connection/minigames-inventory-20260916.json'),JSON.stringify(out,null,2));console.log(JSON.stringify({...out,objects:out.objects.filter(o=>/Microbot|MicroBot|Survival|Flinger|Controller/.test(o.name))},null,2));
