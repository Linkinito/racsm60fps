...
def main():
    ...
    # Verified overrides of cave words, shared by every module using that cave.
    cave_override_path = root/'profiler/generated/cave-overrides.json'
    if cave_override_path.exists():
        overrides = json.loads(cave_override_path.read_text())
        for c in overrides['changes']:
            ci, wi = int(c['cave_index']), int(c['word_index'])
            assert cave_templates[ci][wi] == c['original'], f"Cave override mismatch {ci}/{wi}"
            cave_templates[ci][wi] = c['patched']
    ...
    for p in source['patches']:
        ...
        if p['section'] == '.text':
            flags |= 16
        if p['mechanic'].endswith('VBlank') and LAYERS[p['layer']] == 1 and value == 0 and original >> 26 == 3:
            flags |= 32
        ...
        imported.append(p)
    # Local experimental Metalis additions. Verify the original game words;
    # dynamic actor speeds are deliberately not part of this persistent table.
    # Experimental additions: Metalis discoveries plus the generalisation of the
    # debris and pickup corrections. Every original game word is verified.
    extra_files = ['profiler/generated/metalis-experiments.json',
                   'profiler/generated/generalisation-experiments.json']
    extra_count = 0
    for extra_rel in extra_files:
        ...
        for c in extra['changes']:
            idx = int(c.get('module_index', extra.get('module_index', 4)))
            off, old, new = c['rva'], c['original'], c['patched']
            assert images[idx].word(off) == old, f'Experimental original mismatch L{idx} {off:#x}'
            assert off not in images[idx].relocations, f'Unexpected relocation L{idx} {off:#x}'
            assert not any(x[0] == off for x in patches[idx]), f'Experimental conflict L{idx} {off:#x}'
            code = 16 if off < 0x1D0000 else 0
            patches[idx].append((off, old, new, code, int(c.get('layer', 2))))
            extra_count += 1
    for idx in sorted(patches):
        assert len(patches[idx]) <= 192, f'Capacity exceeded L{idx}'
    ...
    summary = {
        ...
        'experimental_metalis_writes': extra_count, 'experimental_writes': extra_count, 'wrapper_calls_checked': checked_calls, 'caves': len(caves),
