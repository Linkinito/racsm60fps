 ...
 *
 * PPSSPP-first user-mode PRX.  Missing/invalid configuration is DETECT_ONLY.
 * The packaged configuration explicitly enables GLOBAL_60FPS with guarded
 * module-relative base, weapon, breakable and particle corrections.
 * POLICY_MODULE retains the earlier experimental
...
#define RCSM_VERSION "0.6.4-no-menu-dev.1"

/* Startup INI selects FPS. These symbols expose telemetry only: no runtime toggle. */
...
PSP_MODULE_INFO("RCSMProfiler", PSP_MODULE_USER, 0, 6);
...
static uint32_t g_full_caves[RCSM_CAVE_COUNT][RCSM_CAVE_WORDS] __attribute__((aligned(64)));
...
static void apply_config_pair(const char *key, const char *value) {
    ...
    if (string_equal_ci(key, "mode")) {
        ...
        } else if (string_equal_ci(value, "global_60fps")) {
            g_config.mode = RCSM_MODE_GLOBAL_60FPS;
        } else {
            ++g_config.config_errors;
        }
    } else if (string_equal_ci(key, "frame_rate")) {
        ...
        if (parse_u32(value, &parsed) && (parsed & 1u) && !(parsed & ~15u))
            g_config.full_layers = parsed;
        else ++g_config.config_errors;
...
static void load_config(void) {
    ...
    if (size == (int)sizeof(buffer)-1) {
        log_line("config rejected: file exceeds parser capacity; safe mode=detect_only");
        g_config.mode = RCSM_MODE_DETECT_ONLY;
        return;
    }
    if (size <= 0) {
    ...
    /* A naked reference RA is ambiguous across modules. Keep those overrides
       restricted to single-module experiments until they carry module IDs. */
    if (g_config.mode == RCSM_MODE_GLOBAL_60FPS && g_config.ra_override_count != 0u)
        ++g_config.config_errors;
...
/* Restore PPSSPP's cached original instruction before exact patch checks.
   Never accept a JIT marker as a wildcard for a proposed write. */
static int read_patch_word(uint32_t address, uint32_t *value, void *user) {
    int result = read_game_word(address, value, user);
    if (result == 0 && rcsm_is_ppsspp_emuhack(*value)) {
        sceKernelIcacheInvalidateRange((const void *)(uintptr_t)address, 4u);
        result = read_game_word(address, value, user);
    }
    return result;
}


static int prepare_full_patch(const RcsmRuntimeBinding *binding) {
    const RcsmFullModule *m = rcsm_full_module(binding->profile->module_index);
    uint32_t addresses[RCSM_CAVE_COUNT];
    size_t i;
    if (m == NULL) return 0;
    for (i=0; i<RCSM_CAVE_COUNT; ++i) {
        addresses[i] = (uint32_t)(uintptr_t)g_full_caves[i];
        memcpy(g_full_caves[i],g_rcsm_cave_templates[i],sizeof(g_full_caves[i]));
        if (m->cave_return_offsets[i] != 0u) {
            uint32_t target = binding->runtime_base+m->cave_return_offsets[i];
            if (target < binding->runtime_base || target > 0x09FFFFFCu ||
                !rcsm_jal_reachable(addresses[i],target)) return 0;
            g_full_caves[i][g_rcsm_cave_return_indices[i]] = rcsm_encode_j(target);
        }
        /* Accept the legacy and the already-generalised half-step template.
        ...
        sync_game_code(addresses[i],sizeof(g_full_caves[i]),NULL);
    }
    return rcsm_full_prepare(&g_full,m->module_index,binding->runtime_base,
                             addresses,g_config.full_layers);
}

static int full_binding_still_present(void) {
    unsigned matches;
    const RcsmModuleProfile *p;
    if (g_binding.profile == NULL) return 0;
    p = rcsm_identify_module_at_delta(read_patch_word,NULL,g_binding.address_delta,&matches);
    return p != NULL && rcsm_profiles_equivalent(p,g_binding.reference_profile);
}
...
static int apply_runtime_policies(RcsmRuntimeBinding *binding) {
    ...
    if ((g_config.mode != RCSM_MODE_POLICY_MODULE && g_config.mode != RCSM_MODE_GLOBAL_60FPS) ||
        (g_config.allowed_module != 0u && binding->profile->module_index != g_config.allowed_module)) {
        return 1;
    }
    ...
    /* Global family lists apply to the corpus, not to every individual level. */
    for (index = 1u; g_config.mode != RCSM_MODE_GLOBAL_60FPS && index <= RCSM_MAX_FAMILY_ID; ++index) {
...
static int activate_profile(const RcsmRuntimeDetection *detection) {
    ...
    /* Sequences Clank geant (modules 15 et 21) : armer le correctif y fait
       planter PPSSPP.  Reproduit le 17/09/2026 avec la version de base comme
       avec la version generalisation.  Ces deux modules restent en vanilla
       tant que leur profil n'est pas corrige.
       Voir sessions/plantage-clank-geant-20260917-2134/RAPPORT.md. */
    if (detection->reference_profile->module_index == 15u ||
        detection->reference_profile->module_index == 21u) {
        log_line("module excluded key=%s index=%u reason=crash_reported_20260917",
                 detection->reference_profile->module_key,
                 (unsigned)detection->reference_profile->module_index);
        return 0;
    }
    ...
    /* No IO, waiting, or game callback inside this installation transaction. */
    dispatch_state = sceKernelSuspendDispatchThread();
    if (dispatch_state < 0) return 0;
    if (!full_binding_still_present()) {
        (void)sceKernelResumeDispatchThread(dispatch_state);
        return 0;
    }
    if (g_config.mode == RCSM_MODE_GLOBAL_60FPS) {
        full_ok = prepare_full_patch(&g_binding) &&
                  rcsm_full_preflight(&g_full,read_patch_word,NULL);
    }
    if (!full_ok || two_pass_hook_target == 0u || one_pass_hook_target == 0u ||
        ...
        !rcsm_install_policy_redirects(
            ...
            &patch) ||
        (g_config.mode == RCSM_MODE_GLOBAL_60FPS &&
         !rcsm_full_install(&g_full,read_patch_word,write_game_word,sync_game_code,NULL))) {
        ...
        (void)rcsm_full_restore(&g_full,read_patch_word,write_game_word,sync_game_code,NULL);
        (void)sceKernelResumeDispatchThread(dispatch_state);
        if (g_config.mode == RCSM_MODE_GLOBAL_60FPS)
            log_line("full patch refused key=%s address=0x%08X observed=0x%08X",
                     profile->module_key,(unsigned)g_full.bad_address,(unsigned)g_full.bad_word);
    ...
    (void)sceKernelResumeDispatchThread(dispatch_state);
    if (g_full.installed)
        log_line("full patch armed key=%s static_writes=%u pending_constants=%u layers=%u caves=plugin_owned",
                 profile->module_key,(unsigned)g_full.writes,(unsigned)g_full.waiting_constants,g_config.full_layers);
    count_policy_routes(
        &g_binding.patch_view,
        &two_pass_count,
        &one_pass_count,
...
static void deactivate_profile(int restore_if_still_active, const char *reason) {
    ...
    if (restore_if_still_active && g_binding.installed) {
        int dispatch_state = sceKernelSuspendDispatchThread();
        if (dispatch_state >= 0) {
            if (full_binding_still_present()) {
                (void)rcsm_full_restore(&g_full,read_patch_word,write_game_word,sync_game_code,NULL);
                (void)rcsm_remove_policy_redirects(
    ...
    rcsm_fps_leave_module(&g_fps_choice);
...
static RcsmHealthResult check_redirect_health(void) {
    ...
    dispatch_state = sceKernelSuspendDispatchThread();
    if (dispatch_state < 0) return RCSM_HEALTH_FOREIGN;
    if (!full_binding_still_present()) {
        (void)sceKernelResumeDispatchThread(dispatch_state);
        return RCSM_HEALTH_RELOAD;
    }
    redirects_ok = rcsm_classify_policy_redirects(
            &g_binding.patch_view,
            g_binding.two_pass_hook_target,
            g_binding.one_pass_hook_target,
            read_patch_word,
            NULL,
            &state);
    full_status = rcsm_full_health(&g_full,read_patch_word,NULL);
    (void)sceKernelResumeDispatchThread(dispatch_state);
    if (full_status < 0) {
        log_line("full health foreign key=%s address=0x%08X observed=0x%08X",
                 g_binding.profile->module_key,(unsigned)g_full.bad_address,(unsigned)g_full.bad_word);
    ...
    if (state.redirected == g_binding.patch_view.callsite_count && full_status == 1) {
        return RCSM_HEALTH_OK;
    }
    log_line(
...
static void monitor_loop(void) {
    ...
    while (!g_stop_requested) {
        ...
        if (g_binding.profile == NULL) {
            ...
            if (candidate_valid && stable_count >= g_config.stable_polls) {
                if (!activate_profile(&candidate)) {
                    /* An unsuccessful arm must not leave an inactive binding
                       that prevents the blocked/retry logic from running. */
                    memset(&g_binding,0,sizeof(g_binding));
                    memset(&g_full,0,sizeof(g_full));
                    memset(&g_profiler,0,sizeof(g_profiler));
                    g_rcsm_recording_enabled=0u;
            ...
            if (g_full.installed) {
                ...
                if (dispatch_state >= 0) {
                    if (full_binding_still_present())
                        (void)rcsm_full_maintain(&g_full,read_patch_word,write_game_word,sync_game_code,NULL);
                    (void)sceKernelResumeDispatchThread(dispatch_state);
                }
            }
            ++health_counter;
        ...
        g_rcsm_applied_fps = g_fps_choice.applied;
