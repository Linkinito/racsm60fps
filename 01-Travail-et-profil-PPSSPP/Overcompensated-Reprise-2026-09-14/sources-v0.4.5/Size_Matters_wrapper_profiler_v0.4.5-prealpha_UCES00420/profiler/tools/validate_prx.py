...
def parse_stub_source(assembly: str) -> dict[int, int]:
    pattern = re.compile(
        r"RCSM_LEVEL_HOOK\s+rcsm_profiler_hook_l(\d+),\s*"
        r"rcsm_profiler_tail_l\d+,\s*(0x[0-9A-Fa-f]+)"
    )
    return {int(level): int(target, 16) for level, target in pattern.findall(assembly)}
...
def main() -> None:
    ...
    require(source_targets == EXPECTED_TARGETS, "15 cibles source exactement conformes", checks)
    ...
    require(set(module_counts) == set(EXPECTED_TARGETS), "15 modules dans le plan", checks)
    ...
    require(
        "rcsm-wrapper-trace/4" in runtime_source and
        '"rcsm-wrapper-trace/4"' in (profiler / "trace_schema.json").read_text(encoding="utf-8"),
        "export et schéma de trace v4",
        checks,
    )
    require(
    ...
    require(
        "!prepare_tail_jump(profile)" in runtime_source and
        "rcsm_encode_j(runtime_profile->two_pass_entry)" in runtime_source,
        "tail-jump de chaque hook adapté au wrapper runtime",
        checks,
    )
    require(
    ...
    require(
        '#define RCSM_VERSION "0.4.5-prealpha"' in runtime_source,
        "version runtime 0.4.5-prealpha",
        checks,
    )
    require(
        "trace_schema=4" in runtime_source,
        "schéma de trace annoncé dans la ligne de démarrage",
        checks,
    )
    ...
    report: dict[str, object] = {
        ...
        "source_stub_targets": {str(key): f"0x{value:08X}" for key, value in source_targets.items()},
    ...
    if not args.static_only:
        ...
        require(b"0.4.5-prealpha" in prx_path.read_bytes(), "version 0.4.5 intégrée au PRX", checks)
        ...
        for level, target in EXPECTED_TARGETS.items():
        ...
        require(re.search(r"^mode\s*=\s*detect_only\s*$", config_text, re.MULTILINE) is not None,
                "configuration livrée sans écriture", checks)
        require(re.search(r"^allowed_module\s*=\s*1\s*$", config_text, re.MULTILINE) is not None,
                "full_module borné à Pokitaru par défaut", checks)
        require(re.search(r"^focus_family\s*=\s*19\s*$", config_text, re.MULTILINE) is not None,
                "famille focalisée par défaut WF-019", checks)
