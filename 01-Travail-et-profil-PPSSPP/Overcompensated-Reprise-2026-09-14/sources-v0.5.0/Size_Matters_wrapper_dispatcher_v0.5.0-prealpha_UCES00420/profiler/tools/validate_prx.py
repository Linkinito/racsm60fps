...
def main() -> None:
    ...
    require(
        '#define RCSM_VERSION "0.5.0-prealpha"' in runtime_source,
        "version runtime 0.5.0-prealpha",
        checks,
    )
    require(
    ...
    if not args.static_only:
        ...
        require(b"0.5.0-prealpha" in prx_path.read_bytes(), "version 0.5.0 intégrée au PRX", checks)
        ...
        require(re.search(r"^allowed_module\s*=\s*4\s*$", config_text, re.MULTILINE) is not None,
                "module expérimental borné à Metalis", checks)
